...
export default function AdminReturnRequests() {
  ...
  return (
    <div className="admin-return-requests">
      ...
      {loading && (
        <div className="admin-return-requests__state">Loading requests...</div>
      )}
              ...
              {items.length === 0 ? (
                ...
                items.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td>
                      <span
                        className={`admin-return-requests__badge ${getReturnStatusBadgeClass(item.status)}`}
                      >
                    ...
                    <td>
                      {item.requestedAtUtc
                        ? new Date(item.requestedAtUtc).toLocaleString("en-US")
                        : "--"}
                    </td>
                    <td>
                      <Link
                        to={`/admin/return-requests/${item.id}`}
                        className="admin-return-requests__view"
                      >
                        <Eye size={15} /> View
                      </Link>
                    </td>
